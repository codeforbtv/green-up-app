...
const MessageSummary = ({ item, toDetail }: ItemPropsType): React$Element<any> => {
    ...
    return (
        <TouchableOpacity key={item.id} onPress={toDetail}>
            ...
            >
                <Image style={{ width: 80, height: 80 }} source={{ uri: sender.photoURL }} />
...
const MessageSummariesScreen = ({
    actions,
    currentUser,
    messages,
    navigation,
    userHasTeams
}: MessagePropsType): React$Element<any> => {
    ...
    const myMessages = Object.entries(messages)
            ...
            () => (
                <View style={styles.singleButtonHeader}>
                    <View style={styles.headerButton}>
                        <Text style={styles.headerButtonText}>{"To send messages, join or create a team."}</Text>
                    </View>
                </View>
            )
        ],
    ...
    const getContent = R.cond([
        ...
        [
            ...
            _messages => (
                <ListView
                    data={_messages}
                    renderRow={message => <MessageSummary item={message} toDetail={toMessageDetail(message)} />}
                />
            )
        ]
    ]);
...
const mapStateToProps = (state: Object): Object => {
    ...
    const addTeamNamesToMessages = R.compose(
        ...
        R.map((message: Object): MessageType =>
            Message.create({
                ...message,
    ...
    const messages = Object.values((state.messages || {}).messages || {}).reduce(
        (obj: Object, queue: Object): MessageHashType => ({ ...obj, ...addTeamNamesToMessages(queue) }),
        {}
    );
