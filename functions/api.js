...
app.get("/supply_distribution_sites", async (req, res) => {
    ...
    const permittedTowns = isCoordinator ? userPermissions.data().towns || [] : [];
    const filterEditable = R.cond([
        [editable => typeof editable === "string" && editable.toLowerCase() === "false", () => R.filter(supplyDistributionSite => !permittedTowns.includes(trashCollectionSite.townId))],
        [editable => typeof editable === "string" && editable.toLowerCase() === "true", () => R.filter(supplyDistributionSite => permittedTowns.includes(trashCollectionSite.townId))],
        [editable => editable === false, () => R.filter(supplyDistributionSite => !permittedTowns.includes(supplyDistributionSite.townId))],
        [editable => editable === true, () => R.filter(supplyDistributionSite => permittedTowns.includes(supplyDistributionSite.townId))],
        [R.T, () => R.filter(R.T)]
    ]);
    const filterAll = R.compose(filterByName, filterEditable(req.query.editable));
